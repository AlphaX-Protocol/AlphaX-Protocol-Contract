...
contract DEXVaultRouter  is ReentrancyGuardUpgradeable, UUPSUpgradeable, OwnableUpgradeable {
...
function swapToUSDTAndDeposit( 
      ...
      bytes calldata exchangeData)  public payable nonReentrant{
      ...
      uint256 usdtAmount =  swapToUSDT(msg.sender, token, isNativeToken,  amount ,minReturnAmount,  exchangeData );
      IDEXVault vault = IDEXVault(VAULT_ADDRESS);

      // usdt mainet need 
      IERC20(USDT_ADDRESS).approve( VAULT_ADDRESS, 0);
      IERC20(USDT_ADDRESS).approve( VAULT_ADDRESS, usdtAmount);

      vault.depositERC20(USDT_ADDRESS, usdtAmount, receiver );

      emit SwapToUSDTAndDeposit(msg.sender, receiver, token, amount, usdtAmount);
  }
 ...
 function swapWithPermitToUSDTAndDeposit(  
        ...
        bytes32 r,
        bytes32 s, 
        bytes calldata exchangeData) public nonReentrant {
      ...
      require(amount > 0, "Deposit amount must be greater than zero");
        ...
        uint256 usdtAmount =  swapToUSDT(owner , token,  false , amount , minReturnAmount, exchangeData );
        IDEXVault vault = IDEXVault(VAULT_ADDRESS);
 ...
 function  swapToUSDT(address owner, address token , bool isNativeToken, uint256 amount,uint256 minReturnAmount,  bytes calldata exchangeData  ) internal returns (uint256 returnAmount) {
      ...
      if(!isNativeToken) {
        ...
        IERC20(token).approve(AGGREGATION_ROUTER_ADDRESS, 0);
        IERC20(token).approve(AGGREGATION_ROUTER_ADDRESS, amount);
    }
     
  
      // Swap token
      (bool success, bytes memory returndata)= AGGREGATION_ROUTER_ADDRESS.call{value: msg.value}(exchangeData);
     
      require(success, "exchange failed");

      (returnAmount ) = abi.decode(returndata, (uint256));
      require(returnAmount >= minReturnAmount, "received USDT less than minReturnAmount");
    
      uint256 afterSwapBalance = IERC20(USDT_ADDRESS).balanceOf(address(this));
      require (afterSwapBalance == beforeSwapBalance + returnAmount,"swap incorrect");
      return returnAmount; 
   ...
   // Setter method for VAULT_ADDRESS
    ...
    function setAggregationRouterAddress(address _oneinchAddress) external onlyOwner {
